...
root-owned (the container runs as root). The DB lives in the \code{postgres\_data} Docker volume.
Back up both by running \code{./deploy/backup.sh} on the server; it writes a timestamped
\code{gif-storage.tgz} + \code{gifserver-db.sql.gz} under \code{/srv/gif-backups} and deletes
nothing. Copy those off the box for real durability.
\end{note}
