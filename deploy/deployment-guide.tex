...
reverse proxy. Updated 2026-07-11.}
...
\begin{description}[style=nextline,nosep,leftmargin=0pt]
  ...
  \item[\code{deploy/backup.sh}] Backs up the GIF files + database to a timestamped folder, then
        verifies both archives. See Section 11.
  \item[\code{deploy/restore.sh}] Restores a backup produced by \code{backup.sh}. See Section 11.
...
\end{lstlisting}
Excludes the build cache, git history, the local dev data dir, and the env file. Run once with
\code{-n} (dry run) first to preview, then without.

\begin{note}
\textbf{Production GIF storage lives outside the deploy dir} --- it is a bind mount to the host path
in \code{STORAGE_HOST_PATH} (set it to \code{/srv/gif-data}, a sibling of \code{/srv/gif-server}).
Because that path is not under the rsync target, a \code{--delete} deploy cannot reach it regardless
of excludes; because it is a bind mount and not a Docker volume, \code{docker compose down -v},
\code{docker volume prune}, and \code{docker volume rm} cannot reach it either. Only an explicit
\code{rm} on \code{/srv/gif-data} deletes it. The \code{--exclude /data/} above is now only hygiene,
skipping the git-ignored local dev data dir so it is not pushed. Still anchor directory excludes with
a leading slash (\code{/data/}, not \code{data/}): an earlier unanchored \code{--exclude storage/}
silently dropped the \code{src/storage/} \emph{module} and broke the build (\code{error[E0583]: file
not found for module storage}).
\end{note}
...
# REQUIRED in production: put the GIF files OUTSIDE the deploy dir.
STORAGE_HOST_PATH=/srv/gif-data
...
\end{lstlisting}
\begin{itemize}[nosep]
  \item \code{STORAGE_HOST_PATH} is what puts the GIF files at \code{/srv/gif-data}, a \emph{sibling}
        of the deploy dir. \textbf{Do not omit it.} Compose falls back to \code{./data}, which resolves
        \emph{inside} \code{/srv/gif-server} --- the exact thing the safety note in Section 4 says the
        production layout avoids.
...
\begin{description}[style=nextline,nosep,leftmargin=0pt]
  \item[Redeploy after code change] rsync (Step 1), then \code{docker compose up -d --build}. The
        \code{--build} is required: the binary is compiled \emph{into} the image, so
        \code{docker compose restart} re-runs the \emph{old} image and will not pick up new code.
  \item[View app logs] \code{docker compose logs -f app}
  \item[Restart app only] \code{docker compose restart app} --- restarts the current image; use it to
        clear a wedged process, \emph{not} to deploy code.
  \item[Stop everything] \code{docker compose down} --- GIF files are unaffected (they are a host
        bind mount, not a Docker volume); the \code{postgres\_data} volume also persists. \code{down
        -v} deletes \code{postgres\_data} but still cannot touch the bind-mounted GIF files.
\end{description}

\section*{11.\quad Backup and restore}
GIF files live on the host at \code{STORAGE_HOST_PATH} (\code{/srv/gif-data} in production),
root-owned (the container runs as root). The database lives in the \code{postgres\_data} Docker
volume. The two are coupled: rows reference GIF files by hash, so they must be captured and restored
\textbf{together}.

\textbf{Back up} (writes a new timestamped folder under \code{/srv/gif-backups}; deletes nothing):
\begin{lstlisting}
cd /srv/gif-server
./deploy/backup.sh
\end{lstlisting}
It writes \code{gif-storage.tgz} + \code{gifserver-db.sql.gz}, then verifies both: it compares the
archive's file list against \code{STORAGE_HOST_PATH} and checks the dump for the \code{pg_dump}
completion marker, failing loudly if the dump is truncated. Backups are additive --- prune old ones yourself.

\textbf{Restore} (destructive to current state --- see the box below):
\begin{lstlisting}
cd /srv/gif-server
docker compose stop app        # restore.sh refuses to run while the app is connected
./deploy/restore.sh latest     # or a timestamp, or an explicit path
docker compose up -d app
\end{lstlisting}
\code{restore.sh} verifies the archives \emph{before} touching anything, so a corrupt backup aborts
without disturbing live data. It then moves the current GIF dir aside to
\code{<storage>.pre-restore-<stamp>} (it does not delete it), extracts the backup, drops and
recreates the database, and replays the dump. Delete the preserved directory yourself once you have
verified the result.

\begin{note}
\textbf{Restore both halves from the same folder.} Mixing a dump from one run with files from another
leaves rows pointing at GIF files that do not exist. \textbf{The database is dropped and recreated} ---
data written since the backup is lost. Neither script starts, stops, or recreates a container; both
run \code{pg_dump}/\code{psql} inside the already-running \code{db} container via
\code{docker compose exec}, and both refuse to run if \code{db} is down. \textbf{Copy backups off the
box} --- \code{/srv/gif-backups} is out of reach of an \code{rsync --delete} deploy (it is not under
\code{/srv/gif-server}), but it is on the same disk and does not survive hardware failure.
\end{note}

\section*{12.\quad Troubleshooting}
